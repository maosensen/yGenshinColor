...
// Chart token ladder so every slice follows the active preset hue:
// largest slice gets the primary tone, the rest step lighter.
const chartData = [
  { browser: "Website", visitors: 60, fill: "var(--chart-2)" },
  { browser: "Marketplace", visitors: 20, fill: "var(--chart-3)" },
  { browser: "Affiliate", visitors: 20, fill: "var(--chart-1)" },
];
  ...
  return (
    <Card className="h-full w-full py-6 gap-6">
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-4 px-6">
        <ChartContainer
          config={chartConfig}
          className="mx-auto min-h-0 w-full max-w-[260px] flex-1"
        >
          <PieChart>
            <ChartTooltip
              cursor={false}
