/**
 * RippleGrid — vendored from React Bits (reactbits.dev), MIT license.
 * Source: https://github.com/DavidHDev/react-bits (ts-tailwind variant).
 * Keep edits minimal so upstream updates stay easy to diff in.
 */
...
void main() {
    ...
    // [local] track the rAF id so cleanup can cancel it — upstream never
    // cancels, so on unmount/remount the loop keeps drawing to a lost context
    // and throws (blank/broken canvas + error overlay).
    let rafId = 0;
    ...
    return () => {
      ...
      cancelAnimationFrame(rafId); // [local] stop the loop before losing the context
